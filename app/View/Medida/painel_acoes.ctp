...
<div class="container">
	...
	<div id="TargetGraphic" >
		...
		foreach($medidas as $medidas){
			...
			$medida_titulo = explode(' - ', $medidas['Medida']['titulo']);
			$parametros[] = $medida_titulo[0].'|'.$medidas['Medida']['prioridade'].'|'.$medidas['Medida']['andamento'].'|'.$situacao.'|'.$medidas['Medida']['id'];
		}
	...
	<table cellpadding="0" cellspacing="0" class="footable table table-bordered table-hover table-condensed" id="index">
		<thead>
			<tr>
				...
				<th data-hide="phone,tablet" width='10'><?php echo $this->Paginator->sort('Medida.status', 'Andamento'); ?></th>
				<th data-hide="phone,tablet"><?php echo $this->Paginator->sort('Medida.andamento', '(%)'); ?></th>
				<th data-hide="phone,tablet"><?php echo $this->Paginator->sort('Medida.´pdti_prazo', 'Prazo'); ?></th>
		...
		foreach($medida as $medida){?>
			<tr>
				...
				<td><?php echo $status_medida[$medida['Medida']['status_medida']];?></td>
				<td><?php echo $medida['Medida']['andamento'];?></td>
				<td><?php echo $medida['Medida']['pdti_prazo'];?></td>
