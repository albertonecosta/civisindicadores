...
class AnomaliaController extends AppController {
	...
	public function ajaxPainelAnomalia($idAnomalia) {
		...
		$responsaveis = array();
		$count = 0;
		//Logica para pegar o responsavel pela atividade - Necessario para não usar o recursivo 2 da anomalia, por problemas de performance
		foreach($anomalia['Atividade'] as $atividade){
			
			$responsavel = $this->Usuario->find("all", array('conditions' => array('Usuario.id' => $atividade['responsavel_id'])));
			...
			foreach($responsavel[0]['AtividadeResponsavel'] as $atividadeResponsavel){
				if($atividadeResponsavel['id'] == $atividade['id']){
					$responsaveis[$count]['Responsavel']['atividade_id'] = $atividadeResponsavel['id'];
				}
			}
